...
function createRouter({ publicDir, bridge, auth }) {
  return async function route(req, res) {
    ...
    if (pathname === '/api/auth/login' && req.method === 'POST') {
      ...
      const sessionToken = auth.issueSession();
      res.setHeader('Set-Cookie', buildSessionCookie(sessionToken));
...
function buildSessionCookie(token) {
  return `codex_pocket_session=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax`;
}

function clearSessionCookie() {
  return 'codex_pocket_session=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0';
}
